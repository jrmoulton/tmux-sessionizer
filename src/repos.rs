...
type RepoProviderFn = Box<dyn Fn(&Path) -> Result<RepoProvider>>;
...
impl RepoProvider {
    pub fn open(path: &Path, config: &Config) -> Result<Self> {
        ...
        let providers = vcs_provider_config.map(|provider| match provider {
            VcsProviders::Git => Box::new(|path: &Path| {
                gix::open(path)
                    .map(Self::Git)
                    .change_context(TmsError::GitError)
            }) as RepoProviderFn,

            VcsProviders::Jujutsu => Box::new(|path: &Path| {
        ...
        let results = providers.filter_map(|f| f(path).ok()).take(1);
        results
            .into_iter()
            .next()
